...
export class CurrentConditionsCache {
    ...
    get(stationId: string): Observation | null {
        ...
        if (this.cache[stationId]) {
            ...
            if (
                this.cache[stationId].expiryDate.getTime() -
                    new Date().getTime() >
                0
            ) {
                debug(
                    "Serving current conditions for station id '%s' from cache",
                    stationId,
                );
            ...
            debug(
                "Cache entry for station id '%s' expired. Removed entry from cache",
                stationId,
            );
        } else {
    ...
    update(observation: Observation) {
        this.cache[observation.stationID] = new CurrentConditionsCacheEntry(
            observation,
            config.get('cache.expireTime'),
        );
        debug(
            "Updated cache entry for station id '%s' successfully",
