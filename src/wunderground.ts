        ...
        get(endpointUrl, (res) => {
            if (res.statusCode !== 200) {
                const error = new WundergroundAPIError(
                    res.statusCode,
                    `Request to wunderground api failed with status code ${res.statusCode}`,
                );
                return reject(error);
            }
            if (!/^application\/json/.test(res.headers['content-type'] || '')) {
                const error = new WundergroundAPIError(
                    res.statusCode,
                    `The returned request does not contain json according to the content-type header: ${res.headers['content-type']}`,
                );
                return reject(error);
            }
