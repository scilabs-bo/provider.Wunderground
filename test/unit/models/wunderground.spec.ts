...
} from '../../../src/models/wunderground';
import { WeatherObserved } from '../../../src/models/weather';
import { Point } from '../../../src/models/point';

const exemplaryObservation = JSON.parse(
...
describe('Observation model', () => {
    ...
    it('should be adapted correctly from raw response', () => {
        ...
        expect(obs.windSpeed).to.be.closeTo(
            6.111111111111111,
            0.000000000000001,
        );
        expect(obs.windGust).to.be.closeTo(
            8.611111111111111,
            0.000000000000001,
        );
        ...
        expect((obs.observationTime as Date).getTime()).to.equal(
            new Date('2020-01-14T12:46:10Z').getTime(),
        );
    });
    ...
    it('should map to a valid WeatherObserved model', () => {
        ...
        expect(weather.location).to.deep.equal(
            new Point(
                obs.latitude || 0,
                obs.longitude || 0,
                obs.elevation || 0,
...
describe('CurrentConditionsResponse model', () => {
    it('should create', () => {
        expect(new CurrentConditionsResponse()).to.be.instanceOf(
            CurrentConditionsResponse,
        );
    });

    it('should be adapted correctly from raw response', () => {
        let res = CurrentConditionsResponse.adapt({
            observations: [exemplaryObservation],
        });
