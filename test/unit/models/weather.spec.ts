...
describe('WeatherObserved model', () => {
    it('should create', () => {
        expect(new WeatherObserved('', new Date(), '')).to.be.instanceOf(
            WeatherObserved,
        );
    });
    ...
    it('should be created with a location when a Point is passed', () => {
        expect(
            new WeatherObserved('', new Date(), new Point(0, 0)),
        ).to.have.any.keys('_location');
    });

    it('should be created with an address when a string is passed', () => {
        expect(new WeatherObserved('', new Date(), '')).to.have.any.keys(
            '_address',
        );
    });
    ...
    it('should throw an error on invalid relative humidity', () => {
        ...
        expect(() => (w.relativeHumidity = -0.1)).to.throw(
            ValueError,
            /must be a number between 0 and 1/,
        );
        expect(() => (w.relativeHumidity = 1.1)).to.throw(
            ValueError,
            /must be a number between 0 and 1/,
        );
    });
    ...
    it('should throw an error on invalid wind direction', () => {
        ...
        expect(() => (w.windDirection = -1)).to.throw(
            ValueError,
            /measured in decimal degrees/,
        );
        expect(() => (w.windDirection = 361)).to.throw(
            ValueError,
            /measured in decimal degrees/,
        );
    });
